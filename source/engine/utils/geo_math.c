...
int AABB_IntersectsRay(AABB b, Ray3 ray)
{
...
Mat4 Mat4_CreateShadow(Plane plane, Vec3 light)
{
    Mat4 m;
...
void Frustum_UpdateTransform(Frustum* frustum, short viewWidth, short viewHeight)
{
    ...
    const float tang = tanf((frustum->fov * M_PI / 180.0f) * 0.5f);
    
    const float nearHeight = frustum->near * tang;
