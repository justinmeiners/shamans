...
typedef struct
{
    ...
    SeklFrame* frames; // frame information
    Quat* jointRotations; // the rotation for every joint, at every frame
    SkelMarker* markers; // special timing markers, for example for footsteps
} SkelAnim;
...
typedef struct
{
    ...
    SkelMarker* marker; // is there a marker at this frame?
    int finishedAnim; // is this frame the end of the animation?
    int finishedTransition; // is this frame the end of a transition?
} SkelAnimatorInfo;

extern void SkelAnimatorInfo_Clear(SkelAnimatorInfo* info);
