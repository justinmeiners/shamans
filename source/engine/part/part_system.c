...
void PartSystem_Tick(PartSystem* system)
{
    for (int i = 0; i < PART_SYSTEM_EMITTERS_MAX; ++i)
    {
        ...
        if (emitter->effect != kPartEffectNone && emitter->playing)
        {
            ...
            Mat4 rot;
            Mat4 translate = Mat4_CreateTranslate(emitter->position);
            Quat_ToMatrix(emitter->rotation, &rot);
            Mat4_Mult(&translate, &rot, &emitter->worldMatrix);
            Mat4_Scale(&emitter->worldMatrix, emitter->scale);
        }
    }
}
