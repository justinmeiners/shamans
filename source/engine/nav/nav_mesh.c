...
NavPoly* NavMesh_Raycast(const NavMesh* mesh, Ray3 ray, float* r)
{
    ...
    for (int i = 0; i < mesh->polyCount; ++i)
    {
        ...
        if (!AABB_IntersectsRay(poly->bounds, ray)) continue;
    ...
    while (!feof(file))
    {
        ...
        else if (strstr(command, "polys"))
        {
            for (int i = 0; i < polyCount; ++i)
            {
                ...
                Vec3 min = Vec3_Create(INFINITY, INFINITY, INFINITY);
                Vec3 max = Vec3_Create(-INFINITY, -INFINITY, -INFINITY);
                ...
                for (int j = 0; j < edgeCount; ++j)
                {
                    ...
                    for (int k = 0; k < 3; ++k)
                    {
                        Vec3_Get(min, k) = MIN(Vec3_Get(min, k), Vec3_Get(pa, k));
                        Vec3_Get(min, k) = MIN(Vec3_Get(min, k), Vec3_Get(pb, k));
                        
                        Vec3_Get(max, k) = MAX(Vec3_Get(max, k), Vec3_Get(pa, k));
                        Vec3_Get(max, k) = MAX(Vec3_Get(max, k), Vec3_Get(pb, k));
                    }
                }
                
                for (int k = 0; k < 3; ++k)
                    Vec3_Get(center, k) /= (float)edgeCount;
                
                mesh->polys[i].bounds = AABB_Create(min, max);
